...
namespace PHL
{
	...
	// 如果返回为false， 则两直线平行没有交点
	bool getIntersectionPointOfLines(const cv::Point2f& ptAOnLine1, const cv::Point2f& ptBOnLine1,
		const cv::Point2f& ptAOnLine2, const cv::Point2f& ptBOnLine2, cv::Point2f& intersectionPt);
	...
	public:
		/* 静态函数 */
		static Circle fitCircleLS(const Contourf& points);

		static Circle fitCircleRANSAC(const Contourf& points);
	...
	struct Line
	{
		...
		// TO-DO
		Line(cv::Point2f pt1, cv::Point2f pt2);


		/* Public Function */
		int intersect(Line line, cv::Point2f& cross);

		cv::Point2f projectPtToLine(cv::Point2f pt);

		/* Static Function */
		static Line fittingLineLS(const std::vector<cv::Point2f>& pts);

		static Line fittingLineRANSAC(const std::vector<cv::Point2f>& pts);
